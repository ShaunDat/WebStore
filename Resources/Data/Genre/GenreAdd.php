...
<body>
    ...
    <section>
    <div class="container">
      <form class="form-genre" method="POST" >
        <h1>ADD THE GENRE</h1>
          <div class="form-text">
            <label>Genre Name </label>
            <input type="text" placeholder="" name="GenreName">
          </div>
      ...
      <?php 
            ...
            if(isset($_POST['register'])){
                ...
                $sql="INSERT INTO `genremusic`(`GenreName`) VALUES ('$GenreName')";
        ...
        ?>
        <script>
            const formGenre = document.querySelectorAll('.form-text input')
            const formLabel = document.querySelectorAll('.form-text label')
            for(let i=0; i<1 ;i++){
                formGenre[i].addEventListener("mouseover",function(){
                    formLabel[i].classList.add("focus")
                })
                formGenre[i].addEventListener("mouseout",function(){
                    if(formGenre[i].value ==""){
                        formLabel[i].classList.add("focus")
                    }
                })
            }
